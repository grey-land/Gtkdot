namespace Gtkdot {
	...
	public interface GvcGraphMember : GLib.Object {
	...
	public class GvcGraphNode : Gtk.LayoutChild, GvcGraphMember {
		...
		public GvcGraphNode (Gtk.LayoutManager manager, Gtk.Widget child, Gvc.Node node) {
			...
			// Assign preferred widget's size
			Gtk.Requisition child_req;
			child.get_preferred_size (out child_req, null );
			this.node.safe_set("width", "%g".printf( to_inches( (double) child_req.width ) ), "");
			this.node.safe_set("height", "%g".printf( to_inches( (double) child_req.height ) ), "");
		...
		public Gtk.Allocation get_allocation(bool? update = false) {
			if ( this._alloc == null || update ) {
				...
				double _x = ( val != null ) ? double.parse( val.split(",")[0] ) : 0;
				double _y = ( val != null ) ? double.parse( val.split(",")[1] ) : 0;
				this._alloc = {
					  (int) _x - ( (int) _w / 2 ) + default_margin
					, (int) _y - ( (int) _h / 2 ) + default_margin
					, (int) _w - ( default_margin * 2 )
					, (int) _h - ( default_margin * 2 )
				};
			}
			return this._alloc;
		}
	...
	public class GvcGraphEdge : GLib.Object, GvcGraphMember {

		public Gvc.Edge edge { get; construct; }
		...
		public GvcGraphEdge(Gvc.Edge edge, string from, string to) {
			GLib.Object( edge: edge, from: from, to: to );
		}

		public GvcGraphEdge.new_from_node(Gvc.Edge edge, GvcGraphNode from, GvcGraphNode to) {

			GLib.Object( edge: edge, from: from.name(), to: to.name() );
